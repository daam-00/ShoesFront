...
export class ProfileComponent implements OnInit {
  ...
  showEditDialog: boolean = false; // Aggiunta variabile per controllare il dialogo di modifica
  showAddDialog: boolean = false;  // Aggiunta variabile per controllare il dialogo di aggiunta

  @ViewChild("dialogEdit") dialogEdit: ElementRef | undefined;
  @ViewChild("dialogoAdd") dialogoAdd: ElementRef | undefined;
  ...
  editProduct() {
    if (this.productSelected.length == 0) {
      ...
      this.editing = true;
      this.articleSelected = this.products.find((p) => p.id == this.productSelected[0]);

      console.log(this.articleSelected);
      this.showEditDialog = true; // Imposta la variabile per aprire il dialogo di modifica
    }
  }

  openAddDialog() {
    this.showAddDialog = true; // Imposta la variabile per aprire il dialogo di aggiunta
  }
