...
export class ResultSearchComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.params.subscribe(params => {
      ...
      this.getProductsByValueTitle(encodedValue);
    })
  }

  getProductsByValueTitle(value: string) {
    this.http.post<Product[]>(Util.productServerUrl + '/search/model/' + value, {}).subscribe(result => {
  ...
  addToCart(product: Product) {
    ...
    if (AuthService.getToken("id")) {
      ...
      var item: CartItem = new CartItem(product, 1, product.price);
